// Import the main module and change the UI
...
async function getProfileDetails() {
  try {
    ...
    const postsCount = await user.posts_count;
    const commentsCount = await user.comments_count;
    const profileImage = await user.profile_image;

    // Update the DOM with user details
    document.getElementById("profileName").innerHTML = name;
    document.getElementById("profileUsername").innerHTML = username;
    document.getElementById("profileUser").innerHTML = `${username}'s`;
    document.getElementById("profilePostsCount").innerHTML = postsCount;
    document.getElementById("profileCommentsCount").innerHTML = commentsCount;
    document.getElementById("profileImage").src = `
    ${
      Object.entries(profileImage).length === 0
        ? `./images/1.webp`
        : `${profileImage}`
    }`;
  } catch (error) {
...
async function displayProfilePosts() {
  try {
    ...
    const posts = await response.data.data.reverse();
    document.getElementById("userPosts").innerHTML = "";

    // Iterate through posts and generate HTML content
    await posts.map((post) => {
      ...
      // Show or hide (edit) button based on user ownership
            if(isMyPost){    
            permissionChange =`
            <div class="dropdown  border-0  ms-auto ">
              <button class=" border-0 bg-transparent" type="button" data-bs-toggle="dropdown" aria-label="dropdown" aria-expanded="false"">
          ...
          `
            ...
            document.getElementById("addNewPost").style.setProperty("display", "none", "important");
          };
...
// HTML content for each post
      const content = `
           ...
           <div>
           <span class="moveToProfilePage mx-1 font-weight-bolder " data-post-id="${author.id}"  >
           <a href="#" aria-label="user Profile Pic"><img id="userProfilePic" class="profilePic rounded-circle border border-2  rounded-circle border border-2" alt="" src="
           ${
...
// Add event listener for redirecting to post details page
let redirectToPostDetails = document.querySelectorAll(`.redirectToPostDetails`);
Array.from(redirectToPostDetails).forEach(redirectToPostDetail => {
  redirectToPostDetail.addEventListener('click', () => {
    // Extract the post ID from the clicked element
    const postId = redirectToPostDetail.getAttribute('data-post-id');
    // Redirect to the post details page with the post ID as a parameter
    window.location = `postDetails.html?postId=${postId}`;
  });
});

// Delete My Post Modal
...
Array.from(deleteMyPost).forEach(deleteThisPost => {
  deleteThisPost.addEventListener("click", async function deleteMyPost() {
    try {
      // Extract the post details from the clicked element's attribute
      let postAttribute = deleteThisPost.getAttribute('data-post');
      let post = JSON.parse(decodeURIComponent(postAttribute));
      // Set the post ID in the delete post modal input
...
ConfirmPostDelete.addEventListener("click", async function deleteConfirm() {
  try {
    // Get the post ID from the delete post modal input
    ...
    function deleteFinal() {
      ...
      modalInstance.hide();
      // Refresh profile details and posts after deletion
      getProfileDetails();
      displayProfilePosts();
    ...
    // Call the finalize deletion function
    await deleteFinal();
...
Array.from(editMyPostButtons).forEach(editThisPost => {
  editThisPost.addEventListener("click", async function editMyPost() {
    try {
      // Extract post details from the clicked element's attribute
      let postAttribute = editThisPost.getAttribute('data-post');
      let post = JSON.parse(decodeURIComponent(postAttribute));
      let postId = post.id;
      // Set the post details in the edit post modal input fields
      ...
      postModal.toggle();
      // Refresh profile details and posts after editing
      getProfileDetails();
      displayProfilePosts();
    } catch (error) {
      const message = error.response?.data?.message || "Error adding a new comment";
...
addNewPost.addEventListener("click", async function addNewPost() {
  // Clear input fields and set the modal title for adding a new post
...
// Hide loader after the page content is loaded
